...
programme so transactions can be credited back to this app. There are two
parallel mechanisms:

- **Head-level meta tag.** The Base app id `6a14a1f893151d66ddcd63e8` is
  exported from `lib/builderCode.ts` and rendered into `<head>` as a meta
  tag:
...
- **ERC-8021 dataSuffix on wallet transactions.** Wallet payments include
  the Base Builder Code ERC-8021 attribution suffix. Concretely,
  `<PayWithWalletButton />` builds the USDC `transfer(...)` calldata with
  viem&rsquo;s `encodeFunctionData`, then appends the suffix from
  `lib/builderCode.ts` (`BUILDER_CODE_DATA_SUFFIX`) and submits the
  resulting bytes through wagmi&rsquo;s `useSendTransaction`. USDC ignores
  trailing calldata per the ABI, so the transfer executes exactly as
  before; indexers that scan for the suffix can attribute the transaction
  to this app. The `appendBuilderCodeSuffix(...)` helper is idempotent and
  safely no-ops if the suffix is misconfigured, so the canonical USDC
  transfer can never be broken by attribution.

The Base Pay path (`<PayWithBaseButton />`) is intentionally **not**
modified. We will only add a `dataSuffix` / `builderCode` option there
when the Base Pay SDK exposes a first-class supported parameter.
...
- **Base Pay attribution is meta-tag only.** Wallet transactions carry the
  ERC-8021 dataSuffix; Base Pay SDK calls do not, because the SDK does not
  expose a supported `dataSuffix` parameter today. Revisit when it does.
...
- **ERC-8021 attribution on the Base Pay path** &mdash; once the Base Pay
  SDK exposes a supported `dataSuffix` / `builderCode` parameter. The
  wallet path already carries the suffix today.
